...
public class BaseCommonUtil {
    ...
    private static final Log logger = LogFactory.getLog(BaseCommonUtil.class);
    ...
    /**
     * 根据属性名称，向该属性中添加值（自动获取字段的属性类型，并做相应的转换）
     *
     * @param obj
     * @param attrName
     * @param attrValue
     * @return
     */
    public static Object setObjectValue(Object obj, String attrName,
                                        String attrValue) {
        ...
        if (obj != null && StringUtils.isNotEmpty(attrName)
            ...
            for (int i = 0; i < ms.length; i++) {
                String setName = ms[i].getName();
                if (setName.startsWith("set")) {
                    if (("set" + change(attrName)).equals(setName)) {
                        Class<? extends Object>[] cc = ms[i]
                                .getParameterTypes();
                        if (cc.length == 1) {
